...
static const uint8_t SUBSTITUTION_ARRAY[8][4][16] = {
    {
...
uint8_t INITIAL_PERMUTATION_ARRAY[64] = {
...
uint8_t FINAL_PERMUTATION_ARRAY[64] = {
...
uint8_t KEY1_PERMUTATION_ARRAY[28] = {
    57, 49, 41, 33, 25, 17, 9 , 1 , 58, 50, 42, 34, 26, 18,
    10, 2 , 59, 51, 43, 35, 27, 19, 11, 3 , 60, 52, 44, 36,
};

uint8_t KEY2_PERMUTATION_ARRAY[28] = {
    63, 55, 47, 39, 31, 23, 15, 7 , 62, 54, 46, 38, 30, 22,
    14, 6 , 61, 53, 45, 37, 29, 21, 13, 5 , 28, 20, 12, 4 ,
};

int8_t CONTRACTION_PERMUTATION_ARRAY[48] = {
...
uint8_t EXPANSION_PERMUTATION_ARRAY[48] = {
...
uint64_t DES(uint8_t* destination, char mode, uint8_t* keys8b, uint8_t* source, size_t length) {
    ...
    uint64_t newblock64b = join_8bits_to_64bits(keys8b);

    key_expansion(newblock64b, keys48b);

    for (size_t i = 0; i < length; i += 8) {
        split_64bits_to_32bits(
            initial_permutation(
                join_8bits_to_64bits(source + i)
            ),
            &L, &R
        );
        feistel_cipher(mode, &L, &R, keys48b);
        split_64bits_to_8bits(
            final_permutation(
                join_32bits_to_64bits(L, R)
            ),
            (destination + i)
        );
    }
...
uint64_t expansion_permutation(uint32_t block32b) {
    ...
    for (uint8_t i = 0; i < 48; i++) {
        block48b |= (uint64_t)((block32b >> (32 - EXPANSION_PERMUTATION_ARRAY[i])) & 0x01) << (63 - i);
    }
    return block48b;
}
...
void substitution_6bits_to_4bits(uint8_t* blocks6b, uint8_t* blocks4b) {
    ...
    for (uint8_t i = 0, j = 0; i < 8; i += 2, ++j) {
        ...
        blocks4b[j] = SUBSTITUTION_ARRAY[i][block2b][block4b];

        block2b = extreme_bits(blocks6b[i + 1]);
        block4b = middle_bits(blocks6b[i + 1]);
        blocks4b[j] = (blocks4b[j] << 4) | SUBSTITUTION_ARRAY[i + 1][block2b][block4b];
    }
}
...
uint32_t permutation(uint32_t block32b) {
    ...
    for (uint8_t i = 0; i < 32; ++i) {
        new_block32b |= ((block32b >> (32 - PERMUTATION_ARRAY[i])) & 0x01) << (31 - i);
    }
    return new_block32b;
}
...
uint64_t initial_permutation(uint64_t block64b) {
    ...
    for (uint8_t i = 0; i < 64; ++i) {
        new_block64b |= ((block64b >> (64 - INITIAL_PERMUTATION_ARRAY[i])) & 0x01) << (63 - i);
    }
    return new_block64b;
}
...
uint64_t final_permutation(uint64_t block64b) {
    uint64_t new_block64b = 0;
    for (uint8_t i = 0; i < 64; ++i) {
        new_block64b |= ((block64b >> (64 - FINAL_PERMUTATION_ARRAY[i])) & 0x01) << (63 - i);
    }
    return new_block64b;
}
...
void key_permutation_56bits_to_28bits(uint64_t block56b, uint32_t* block28b_1, uint32_t* block28b_2) {
    for (uint8_t i = 0; i < 28; ++i) {
        *block28b_1 |= ((block56b >> (64 - KEY1_PERMUTATION_ARRAY[i])) & 0x01) << (31 - i);
        *block28b_2 |= ((block56b >> (64 - KEY2_PERMUTATION_ARRAY[i])) & 0x01) << (31 - i);
    }
}
...
uint64_t key_contraction_permutation(uint64_t block56b) {
    uint64_t block48b = 0;
    for (uint8_t i = 0; i < 48; ++i) {
        block48b |= ((block56b >> (64 - CONTRACTION_PERMUTATION_ARRAY[i])) & 0x01) << (63 - i);
    }
    return block48b;
}
